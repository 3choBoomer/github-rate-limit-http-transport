...
func TestNewTransport(t *testing.T) {
	...
	t.Run("defaults", func(t *testing.T) {
		...
		assert.False(t, tr.Limits.fetchUser)
		assert.Empty(t, tr.userLoginHeader)
	})

	t.Run("options", func(t *testing.T) {
		callback := func(*http.Response, Resource, *Rate, *User) {}
		tr := NewTransport(base,
			WithFetchUser(true),
			WithNotifyCallback(callback),
			WithUserLoginHeader("Custom-Key"),
		)

		assert.True(t, tr.Limits.fetchUser)
		assert.NotNil(t, tr.Limits.Notify)
		assert.Equal(t, "Custom-Key", tr.userLoginHeader)
	})

	t.Run("WithUserLoginHeader default", func(t *testing.T) {
		tr := NewTransport(base, WithUserLoginHeader(""))
		assert.Equal(t, "X-GitHub-User-Login", tr.userLoginHeader)
	})
}
...
func TestTransport_RoundTrip(t *testing.T) {
	...
	t.Run("user header injection", func(t *testing.T) {
		login := "octocat"
		...
		t.Run("no header configured", func(t *testing.T) {
			tr := NewTransport(base)
			tr.Limits.StoreUser(user)
			req, _ := http.NewRequest("GET", "https://api.github.com", nil)
			resp, err := tr.RoundTrip(req)
			assert.NoError(t, err)
			assert.Empty(t, resp.Header.Get("X-GitHub-User-Login"))
		})
		...
		t.Run("default header", func(t *testing.T) {
			...
			resp, err := tr.RoundTrip(req)
			assert.NoError(t, err)
			assert.Equal(t, "octocat", resp.Header.Get("X-GitHub-User-Login"))
		})

		t.Run("custom header", func(t *testing.T) {
			tr := NewTransport(base, WithUserLoginHeader("My-User"))
			tr.Limits.StoreUser(user)
			req, _ := http.NewRequest("GET", "https://api.github.com", nil)
			resp, err := tr.RoundTrip(req)
			assert.NoError(t, err)
			assert.Equal(t, "octocat", resp.Header.Get("My-User"))
		})
	})
